...
public class Game implements Runnable
{
    ...
    private void InitGame()
    {
        ...
        Tiles=new TileFactory(new SpriteSheet(ImageLoader.LoadImage("/MegaTileset1.png")));
    ...
    public synchronized void StopGame()
    {
        if(runState == true) {
            ...
            if (gameThread.isInterrupted()) {
                System.err.println("Inchiderea ferestrei a fost intrerupta.");
            }
            if (wnd.GetWndFrame().isDisplayable()) {
                wnd.GetWndFrame().dispose();
            }
        }
        else
    ...
    public TileFactory GetTiles() { return Tiles;}

    public boolean GameStarted() {return gameStarted;}

    public void changeGameStarted(boolean x) {gameStarted = x; }

    public void StartPlay(boolean x) { startPlay=x;}


    public GameWindow GetGameWindow() { return this.wnd;}

    public State GetGameState() {return State.GetState();}
    ...
    public void SetAboutState() {State.SetState(aboutState);}
    public void SetMenuState() {State.SetState(menuState);}
    public void SetSettingsState() {State.SetState(settingsState);}
    public void LoadNewGame() {playState= new PlayState(refLink);}

    public void SetDifferentLanguage() {
        if (language == "romanian")
            language = "english";
        else
            language = "romanian";
        System.out.println("Limba schimbata");
    }
